...
metadata {
    definition(name: 'LIFX Day and Dusk', namespace: 'sriley-claude', author: 'Robert Alan Heyes', importUrl: 'https://raw.githubusercontent.com/steveisgreat/Hubitat/main/lifx/LIFXDayAndDusk.groovy') {
        capability "Bulb"
    ...
    preferences {
        ...
        input "pollInterval", "enum", title: "State polling interval (staggered per device)", options: ["0":"Disabled","1":"1 minute","5":"5 minutes","10":"10 minutes","15":"15 minutes","30":"30 minutes"], defaultValue: "5", required: true
...
private void schedulePoll() {
    Integer minutes = (settings.pollInterval ?: '5') as Integer
    if (minutes <= 0) {
        logInfo 'State polling disabled'
        return
    }
    // Stagger with a random offset so devices don't all poll on the same tick
    Integer offsetSeconds = new Random().nextInt(60)
    Integer offsetMinutes = new Random().nextInt(minutes)
    schedule("${offsetSeconds} ${offsetMinutes}/${minutes} * * * ?", 'poll')
}
