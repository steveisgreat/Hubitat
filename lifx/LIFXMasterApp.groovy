...
def mainPage() {
    dynamicPage(name: "mainPage", title: "Options", install: true, uninstall: true) {
        section {
            ...
            input 'descriptionTextLogging', 'bool', title: 'Enable info logging', defaultValue: true, submitOnChange: true
            input 'debugLogging', 'bool', title: 'Enable debug logging (auto-off after 30 minutes)', defaultValue: false, submitOnChange: true
...
def updated() {
    ...
    if (settings.debugLogging) {
        log.info 'Debug logging will be disabled in 30 minutes'
        runIn(1800, 'logsOff')
    }
    initialize()
}
...
//    def discoveryDevice = addChildDevice 'sriley-claude', 'LIFX Discovery', 'LIFX Discovery'
...
List<Map> parseForDevice(device, String description, Boolean displayed, Boolean updateDevice = false) {
    ...
    switch (header.type) {
        ...
        case messageType['LIGHT.STATE']:
            ...
            List<Map> result = []
            addIfChanged(result, device, "level", intScaleDown100(data.color.brightness), displayed)
            if (device.hasCapability('Color Control')) {
                addIfChanged(result, device, "hue", intScaleDown100(data.color.hue), displayed)
                addIfChanged(result, device, "saturation", intScaleDown100(data.color.saturation), displayed)
            }
            if (device.hasCapability('Color Temperature')) {
                addIfChanged(result, device, "colorTemperature", data.color.kelvin as Integer, displayed)
            }
            if (device.hasCapability('Switch')) {
                addIfChanged(result, device, 'switch', (data.power == 65535) ? 'on' : 'off', displayed)
            }
            return result
        ...
        case messageType['DEVICE.STATE_POWER']:
            ...
            List<Map> powerEvents = []
            addIfChanged(powerEvents, device, "switch", (data.powerLevel as Integer == 0 ? "off" : "on"), displayed, [data: [syncing: "false"]])
            return powerEvents
        case messageType['LIGHT.STATE_POWER']:
            Map data = parsePayload 'LIGHT.STATE_POWER', header
            List<Map> lightPowerEvents = []
            addIfChanged(lightPowerEvents, device, "switch", (data.powerLevel as Integer == 0 ? "off" : "on"), displayed, [data: [syncing: "false"]])
            addIfChanged(lightPowerEvents, device, "level", (data.powerLevel as Integer == 0 ? 0 : 100), displayed, [data: [syncing: "false"]])
            return lightPowerEvents
/*
...
/** Only emit an event when the value actually changed, to avoid a per-poll event storm */
private void addIfChanged(List<Map> result, device, String name, value, boolean displayed, Map extra = null) {
    if (device.currentValue(name)?.toString() != value?.toString()) {
        Map event = [name: name, value: value, displayed: displayed]
        if (extra) {
            event.putAll(extra)
        }
        result << event
...
private void logInfo(msg) {
    if (settings.descriptionTextLogging != false) {
        log.info msg
    }
}
