...
> **Modified fork (namespace `sriley-claude`).** This is a personal fork of
> [robheyes/lifxcode](https://github.com/robheyes/lifxcode) with efficiency changes to
> reduce network and log "chattiness":
> * **Configurable, staggered polling.** Each driver now has a *State polling interval*
>   preference (Disabled / 1 / 5 / 10 / 15 / 30 min, default **5 min** instead of a fixed
>   1 min) and applies a random per-device offset so all devices no longer poll on the same tick.
> * **Change-only events.** The app now emits `level`/`hue`/`saturation`/`colorTemperature`/
>   `switch` events only when the value actually changed, eliminating the per-poll event storm.
> * **Gated logging.** The Master App's debug logging is off by default, toggled from its
>   settings page, and auto-disables after 30 minutes. Driver activity logging defaults to off.
>
> All credit for the original integration goes to Robert Alan Heyes and contributors.
